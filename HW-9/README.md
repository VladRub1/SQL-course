...
Во-первых, видим, что последующие такие же запросы
выполнялись в 2-3 раза быстрее без изменения числа ресурсов на операцию (cost).
...
Поэтому, как было сказано в задании, в дальнейшем в рамках запроса к ним можно
обращаться как к материализованному результату.
...
Попробую для эксперимента посмотреть на план каждого из двух `CTE`-запросов по отдельности.
Вывожу результат:
```
...
`CTE`-запрос, чтобы отфильтровать нужные строки после выполнения оконной функции, 
но он не должен тратить много времени на себя.
